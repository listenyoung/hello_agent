...
class MySimpleAgent(SimpleAgent):
    ...
    def _run_with_tools(self,messages:list,input_text:str,max_tool_interations:int,**kwargs)->str:
        ...
        final_response = ""
        while current_interation < max_tool_interations:
            #调用LLM
            response = self.llm
            # 检查是否有工具调用
            ...
            if tool_calls:
                ...
                for call in tool_calls:
                    results = self._execute_tool_call(call['tool_name'],call['parameters'])
            ...
            break
        # 如果超过最大迭代次数，获取最后一次回答
        if current_interation >= max_tool_interations and not final_response:
